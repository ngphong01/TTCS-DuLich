...
// POST /api/tour/:id/generate-description - Generate tour description using Anthropic
// Ví dụ sử dụng Anthropic Client trong route
router.post('/:id/generate-description', authRequired, isAdmin, async (req, res) => {
  try {
    const tourId = parseInt(req.params.id);
    ...
    // Tạo prompt cho Anthropic
    const prompt = `Hãy viết mô tả hấp dẫn cho tour du lịch sau:
- Tên tour: ${tour.name}
- Giá: ${tour.price ? tour.price.toLocaleString('vi-VN') + ' VNĐ' : 'Chưa có'}
- Thời gian: ${tour.duration || 'N/A'} ngày
- Điểm đến: ${tour.destination?.name || 'N/A'} (${tour.destination?.country || 'N/A'})
${tour.shortDescription ? `- Mô tả hiện tại: ${tour.shortDescription}` : ''}
...
- Viết bằng tiếng Việt`;
    ...
    try {
      generatedDescription = await chatWithAnthropic(prompt, [], {
        max_tokens: 500,
        system: 'Bạn là copywriter chuyên nghiệp cho công ty du lịch TravelGo. Bạn chuyên viết mô tả tour hấp dẫn và chuyên nghiệp.'
      });
    } catch (aiError) {
      console.error('Anthropic API error:', aiError);
      return res.status(500).json({ 
        message: 'Lỗi khi tạo mô tả bằng AI',
        error: process.env.NODE_ENV === 'development' ? aiError.message : undefined
      });
    }

    // Trả về kết quả (không tự động cập nhật, để admin xem trước)
    res.json({
      success: true,
      tourId: tourId,
      tourName: tour.name,
      generatedDescription: generatedDescription,
      message: 'Mô tả đã được tạo. Bạn có thể copy và cập nhật vào tour.'
    });

  } catch (error) {
    console.error('Error generating tour description:', error);
    res.status(500).json({ 
      message: 'Error generating tour description',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
});
