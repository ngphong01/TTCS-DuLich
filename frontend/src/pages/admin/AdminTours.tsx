...
import { createTour, deleteTour, getToursPaged, updateTour } from '../../services/tour';
...
// Form input type (what comes from HTML form - strings)
type FormInput = {
  name: string;
  slug: string;
  description?: string;
...
// Output type (what we need for API)
type TourFormData = {
  name: string;
  slug: string;
  description?: string;
...
// Schema for validation (accepts form input)
const schema = z.object({
  ...
  adultPrice: z.union([z.string(), z.number()]).optional(),
  childPrice: z.union([z.string(), z.number()]).optional(),
  duration: z.union([z.string(), z.number()]).default('1'),
  destinationId: z.union([z.string(), z.number()]).optional(),
});

type PageResult<T = any> = { items: T[]; total: number; page: number; pages: number };
...
export default function AdminTours() {
  ...
  const { data, isLoading, refetch } = useQuery<PageResult>({
    queryKey: ['admin', 'tours', page, pageSize],
    queryFn: () => getToursPaged(page, pageSize),
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<FormInput>({
  ...
  // Transform form input to TourFormData
  const transformToTourFormData = (input: FormInput): TourFormData => {
    ...
    return {
      ...
      adultPrice: parseNumber(input.adultPrice),
      childPrice: parseNumber(input.childPrice),
      duration: typeof input.duration === 'string' 
        ? (input.duration === '' ? 1 : Number.isNaN(Number(input.duration)) ? 1 : Number(input.duration))
        : (input.duration ?? 1),
      destinationId: parseNumber(input.destinationId),
    };
  };

  async function onCreate(input: FormInput) {
    try {
      // Validate input
      schema.parse(input);
      // Transform to TourFormData
      const values = transformToTourFormData(input);
  ...
  async function onUpdate(id: number, patch: Partial<TourFormData>) {
    try {
      await updateTour(id, patch);
  ...
  async function onDelete(id: number) {
    ...
    try {
      ...
      if (errorMsg.includes('foreign key') || errorMsg.includes('booking')) {
        toast.error('Không thể xóa tour này vì đã có đặt chỗ. Vui lòng xóa các đặt chỗ trước.');
      } else {
  ...
  const filteredItems = useMemo(() => {
    ...
    return data.items.filter((t: any) => 
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 lg:p-8 border border-white/20">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            >
              ...
              <span className="text-sm md:text-base">Thêm Tour</span>
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Tổng tour</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {data.items.filter((t: any) => t.featured).length}
                  </p>
                </div>
            ...
            <form onSubmit={handleSubmit(onCreate)} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    placeholder="VD: tour-da-lat-3n2d"
                    {...register('slug')}
                  />
                  {errors.slug && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                ...
                <div>
                  ...
                  {errors.adultPrice && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                      <XMarkIcon className="h-3 w-3" />
                      {errors.adultPrice.message as any}
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.childPrice && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                      <XMarkIcon className="h-3 w-3" />
                      {errors.childPrice.message as any}
                    </p>
                  )}
                </div>
        ...
        {!isLoading && filteredItems.length === 0 && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 text-center border border-white/20">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-2">
              {searchTerm ? 'Không tìm thấy kết quả' : 'Chưa có tour nào'}
            </h3>
            <p className="text-gray-600">
              {searchTerm ? 'Thử tìm kiếm với từ khóa khác' : 'Bắt đầu thêm tour mới'}
            </p>
          </div>
        )}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b-2 border-blue-200">
                      ...
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Tên tour</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden md:table-cell">Giá (VND)</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden lg:table-cell">Thời lượng</th>
                    ...
                    {filteredItems.map((t: any) => (
                      <tr key={t.id} className="hover:bg-blue-50/50 transition-colors group">
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4">
                          <span className="font-mono font-bold text-xs md:text-sm text-blue-600">#{t.id}</span>
                        </td>
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden sm:table-cell">
                          <div className="flex items-center justify-center relative">
                            ...
                            <div 
                              className={`img-fallback w-16 h-16 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md ${t.image ? 'hidden' : ''}`}
                            >
                        ...
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4">
                          <InlineEdit
                            value={t.name}
                            onSave={(v) => onUpdate(t.id, { name: v })}
                          />
                        </td>
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden md:table-cell">
                          <InlineEdit
                            value={((t.adultPrice || t.price) ?? 0).toLocaleString('vi-VN')}
                            onSave={(v) => {
                        ...
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden md:table-cell">
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                              type="checkbox"
                              checked={!!t.featured}
                              onChange={(e) => onUpdate(t.id, { featured: e.target.checked })}
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 px-6 py-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="text-sm text-gray-700">
                  ...
                  <span className="font-bold text-blue-600">{total}</span> tour
                </div>
