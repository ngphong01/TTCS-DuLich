...
import { createHotel, deleteHotel, getHotelsPaged, updateHotel } from '../../services/hotel';
...
export default function AdminHotels() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<FormData>({
    defaultValues: { name: '', slug: '', description: '', featured: false, pricePerNight: 0, city: '', country: 'Việt Nam', address: '' },
  });
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      const response = await fetch('/api/upload/hotel', {
        method: 'POST',
        headers: {
  ...
  async function onCreate(values: FormData) {
    try {
      ...
      if (err?.errors) {
        ...
        toast.error(err?.response?.data?.message || 'Không thể tạo khách sạn');
      }
    }
  }
      ...
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1 text-sm md:text-base hidden sm:block">Tạo và quản lý các khách sạn</p>
              </div>
            </div>
            <button
            ...
            >
              ...
              <span className="text-sm md:text-base">Thêm Khách Sạn</span>
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Tổng khách sạn</p>
            ...
            <form onSubmit={handleSubmit(onCreate)} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    placeholder="VD: khach-san-muong-thanh"
                    {...register('slug')}
                  />
                  {errors.slug && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                ...
                <div>
                  ...
                  {errors.pricePerNight && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                      <XMarkIcon className="h-3 w-3" />
                      {errors.pricePerNight.message as any}
                    </p>
                  )}
                </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 border border-white/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Tìm kiếm theo tên, slug hoặc thành phố..."
              value={searchTerm}
        ...
        {!isLoading && filteredItems.length === 0 && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 text-center border border-white/20">
            ...
            <p className="text-gray-600">
              {searchTerm ? 'Thử tìm kiếm với từ khóa khác' : 'Bắt đầu thêm khách sạn mới'}
            </p>
          </div>
        )}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b-2 border-blue-200">
                      ...
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Tên khách sạn</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden md:table-cell">Thành phố</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden lg:table-cell">Giá/đêm (VND)</th>
                      ...
                      <tr key={d.id} className="hover:bg-blue-50/50 transition-colors group">
                        ...
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden sm:table-cell">
                          <div className="flex items-center justify-center relative">
                            ...
                            <div 
                              className={`img-fallback w-16 h-16 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md ${d.image ? 'hidden' : ''}`}
                            >
                        ...
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden lg:table-cell">
                          <InlineEdit
                            value={(d.pricePerNight ?? 0).toLocaleString('vi-VN')}
                            onSave={(v) => {
                              ...
                              onUpdate(d.id, { pricePerNight: numValue });
                            }}
                          />
                        </td>
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden md:table-cell">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 px-6 py-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="text-sm text-gray-700">
                  ...
                  <span className="font-bold text-blue-600">{total}</span> khách sạn
                </div>
