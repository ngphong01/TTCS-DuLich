    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Quản lý tất cả đơn đặt tour</p>
        </div>
      ...
      {!isLoading && data && (
        <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-6 text-white">
          <p className="text-sm opacity-90 mb-1">Tổng số đơn đặt chỗ</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div className="flex gap-4 items-center flex-wrap">
          <div>
            ...
            <select 
              className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              value={sortBy} 
      ...
      {!isLoading && data && (
        <>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <Table headers={['ID', 'Người dùng', 'Điểm đến', 'Trạng thái', 'Tổng tiền', 'Ngày tạo']}>
              {data.items.map((b: any) => (
                <tr key={b.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{b.user?.email || '-'}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{b.destination?.name || '-'}</td>
      ...
      {!isLoading && data && data.items.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">Chưa có đơn đặt chỗ nào</p>
        </div>
      )}
    </div>
  );
