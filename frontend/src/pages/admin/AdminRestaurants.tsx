...
import { createRestaurant, deleteRestaurant, getRestaurantsPaged, updateRestaurant } from '../../services/restaurant';
...
export default function AdminRestaurants() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<FormData>({
    defaultValues: { name: '', slug: '', description: '', featured: false, city: '', country: 'Việt Nam', address: '', cuisine: '', priceRange: '$$' },
  });
  ...
  async function onCreate(values: FormData) {
    try {
      ...
      if (err?.errors) {
        ...
        toast.error(err?.response?.data?.message || 'Không thể tạo nhà hàng');
      }
    }
  }
  ...
  async function onDelete(id: number) {
    if (!confirm('Bạn có chắc chắn muốn xóa nhà hàng này?')) return;
    try {
  ...
  const filteredItems = useMemo(() => {
    ...
    return data.items.filter((d: any) => 
      ...
      (d.cuisine && d.cuisine.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  }, [data?.items, searchTerm]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 p-3 md:p-4 lg:p-6">
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 lg:p-8 border border-white/20">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2 md:gap-4">
              <div className="bg-gradient-to-br from-orange-500 to-red-500 p-3 md:p-4 rounded-xl md:rounded-2xl shadow-lg flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1 text-sm md:text-base hidden sm:block">Tạo và quản lý các nhà hàng</p>
              </div>
            </div>
            <button
              onClick={() => setShowCreateForm(!showCreateForm)}
              className="w-full sm:w-auto px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl hover:from-orange-600 hover:to-red-600 transition-all font-semibold shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
            >
              <PlusIcon className="h-4 w-4 md:h-5 md:w-5" />
              <span className="text-sm md:text-base">Thêm Nhà Hàng</span>
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Tổng nhà hàng</p>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Nổi bật</p>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Thường</p>
                  <p className="text-3xl font-bold text-gray-900">
                    {data.items.filter((d: any) => !d.featured).length}
                  </p>
                </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-white/20">
            ...
            <form onSubmit={handleSubmit(onCreate)} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="VD: Nhà hàng Ngọc Mai"
                    {...register('name')}
                  />
                  {errors.name && (
                ...
                <div>
                  ...
                  <input
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
                    placeholder="VD: nha-hang-ngoc-mai"
                    {...register('slug')}
                  />
                  {errors.slug && (
                    <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
                    placeholder="VD: Việt Nam, Nhật Bản, Ý..."
                    {...register('cuisine')}
                  />
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    <label className="flex-1 cursor-pointer">
                      ...
                      />
                      <div className="w-full border-2 border-dashed border-gray-300 rounded-xl px-4 py-6 text-center hover:border-orange-400 hover:bg-orange-50 transition-all">
                        {imagePreview ? (
              ...
              <div>
                ...
                <textarea
                  className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all resize-none"
                  rows={4}
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-gray-200">
                <button
                  ...
                  type="submit"
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl hover:from-orange-600 hover:to-red-600 transition-all font-semibold shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 border border-white/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Tìm kiếm theo tên, slug, thành phố hoặc loại ẩm thực..."
              value={searchTerm}
        ...
        {!isLoading && filteredItems.length === 0 && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 text-center border border-white/20">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-2">
              {searchTerm ? 'Không tìm thấy kết quả' : 'Chưa có nhà hàng nào'}
            </h3>
            <p className="text-gray-600">
              {searchTerm ? 'Thử tìm kiếm với từ khóa khác' : 'Bắt đầu thêm nhà hàng mới'}
            </p>
          </div>
        )}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-orange-50 to-red-50 border-b-2 border-orange-200">
                      ...
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Tên nhà hàng</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden md:table-cell">Loại</th>
                      <th className="px-2 md:px-4 lg:px-6 py-3 md:py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider hidden lg:table-cell">Thành phố</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredItems.map((d: any) => (
                      <tr key={d.id} className="hover:bg-orange-50/50 transition-colors group">
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4">
                          <span className="font-mono font-bold text-xs md:text-sm text-orange-600">#{d.id}</span>
                        </td>
                        <td className="px-2 md:px-4 lg:px-6 py-3 md:py-4 hidden sm:table-cell">
                          <div className="flex items-center justify-center relative">
                            ...
                            <div 
                              className={`img-fallback w-16 h-16 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md ${d.image ? 'hidden' : ''}`}
                            >
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 px-6 py-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="text-sm text-gray-700">
                  ...
                  <span className="font-bold text-orange-600">{total}</span> nhà hàng
                </div>
...
function InlineEdit({ value, onSave }: { value: string; onSave: (v: string) => Promise<void> | void }) {
  ...
  return editing ? (
    ...
    >
      <input
        className="border-2 border-orange-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
        value={val}
    ...
    <button
      className="text-left w-full font-medium text-gray-900 hover:text-orange-600 transition-colors flex items-center gap-2 group"
      onClick={() => setEditing(true)}
    >
      <span>{value}</span>
      <PencilIcon className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity text-orange-500" />
    </button>
  );
}
