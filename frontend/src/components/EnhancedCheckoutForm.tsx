...
export default function EnhancedCheckoutForm({
  ...
  const { data: session } = useSimpleAuth() as { data: { user?: { email?: string } } | null };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bookingData = {
        ...
        discountAmount: appliedCoupon?.discountAmount || appliedCoupon?.discount || 0,
        serviceFee,
        tax: (basePrice * guests) * (taxRate / 100),
        totalAmount: basePrice * guests + serviceFee + ((basePrice * guests) * (taxRate / 100)) - (appliedCoupon?.discountAmount || appliedCoupon?.discount || 0) + paymentMethodFee
      };
